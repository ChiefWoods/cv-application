...
export default function FormMonthInput({ value, onChange }) {
  ...
  function formatValueToDate(value) {
    const date = parse(value, "MMM, yyyy", new Date());
  ...
  useEffect(() => {
    ...
    const newValue = `${formatDateToValue(startDate)} - ${isCurrentRole ? "Present" : formatDateToValue(endDate)}`;
